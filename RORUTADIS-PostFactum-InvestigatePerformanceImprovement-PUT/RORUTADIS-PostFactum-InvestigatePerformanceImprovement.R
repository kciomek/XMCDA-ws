# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTADIS-PostFactum-InvestigatePerformanceImprovement.R
...
if (length(fileErrors) == 0) {
  ...
  if (is.null(dataError)) {
    tmpErr <- try({
      ...
      improvementResult <- improveAssignment(which(alternativesIDs == alternative),
...
if (is.null(errorMessage)) {
  ...
  if (is.null(improvementResult))
    putLogMessage(tree, "no", name = "isImprovementPossible")
  else
    putLogMessage(tree, "yes", name = "isImprovementPossible")
} else {
