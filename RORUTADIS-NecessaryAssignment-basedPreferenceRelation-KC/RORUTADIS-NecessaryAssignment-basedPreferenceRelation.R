# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTADIS-NecessaryAssignment-basedPreferenceRelation.R
...
if (length(fileErrors) == 0) {
  ...
  if (is.null(dataError)) {
    tmpErr <- try({
      ...
      comparisons <- compareAssignments(problem, TRUE)
      comparisonsMatrix <- matrix(ncol = 3, nrow = 0)
      for (i in 1:nrAlternatives) {
        for (j in 1:nrAlternatives) {
          if (i != j) {
            if (comparisons[i, j]) {
              comparisonsMatrix <- rbind(comparisonsMatrix, c(alternativesIDs[i], alternativesIDs[j], "0"))
            }
          }
        }
      }
    ...
    if (inherits(tmpErr, 'try-error')) {
      ...
      newXMLNode("xmcda:XMCDA", 
                 ...
                 parent = tree)
      putAlternativesComparisonsLabels(tree, comparisonsMatrix, "atLeastAsGoodAs")
      saveXML(tree, file = "assignmentComparisons.xml")
    }
  }
  else {
