# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTADIS-ExtremeClassCardinalities.R
...
if (length(fileErrors) == 0) {
  ...
  if (is.null(dataError)) {
    tmpErr <- try({
      ...
      extremeCardinalities <- calculateExtremeClassCardinalities(problem)
      extremeCardinalities <- cbind(1:nrCategories, extremeCardinalities)
    })
    ...
    if (inherits(tmpErr, 'try-error')) {
      ...
      newXMLNode("xmcda:XMCDA", 
                 ...
                 parent = tree)
      putCategoriesIntervalValues(tree, extremeCardinalities, categoriesIDs, NULL)
      saveXML(tree, file = "categoriesExtremeCardinalities.xml")
    }
  }
  else {
