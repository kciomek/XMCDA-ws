# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < RORUTADIS-PostFactum-InvestigateValueChange.R
...
if (length(fileErrors) == 0) {
  ...
  if (is.null(dataError)) {
    tmpErr <- try({
      ...
      criteriaFunctions <- getCharacteristicPoints(problem, investigationResult$solution)
      names(criteriaFunctions) <- criteriaIDs
      assignments <- getAssignments(problem, investigationResult$solution)
